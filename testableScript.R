...
for(i in 1:length(files)){
   ...
   j$other1 <- j[2,6]         # get other 1
   j$other2 <- j[2,7]         # get other 2
   j$other3 <- j[2,8]         # get other 3
   j$other4 <- j[2,9]         # get other 4
   j$other5 <- j[2,10]        # get other 5
   j$browser <- j[2,11]       # get browser
   j$version <- j[2,12]       # get version
   j$screenWidth <- j[2,13]   # get screen width
   j$screenHeight <- j[2,14]  # get screen height
   j$OS <- j[2,15]            # get operating system
   j$OS_lang <- j[2,16]       # get operating system language
   j$calibration <- j[2,17]   # get calibration
   j$ip <- j[2,18]            # get IP address
   j$code <- j[2,19]          # get completion code
